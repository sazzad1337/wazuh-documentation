...
#. Click **Create Role mapping** and complete the empty fields with the requested information.
      ...
      - **Role mapping name**: Assign a name to the role mapping.
      - **Roles**: Select the role created previously, for example ``Team_A_role``, and the ``cluster_readonly`` role. This role assigns the user basic configuration reading permissions.
      - **Internal users**: Select the internal user created previously.
